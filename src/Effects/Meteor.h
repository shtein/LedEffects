...
inline uint8_t Meteor::getHue() const{
  return _hue;
}
...
class EffectMeteorRain: public Effect{
  protected:
    void reset(){    
      ...
      for(uint16_t i = 0; i < METEOR_COUNT; i++){
        //Ret meteor position
    ...
    void proceed(CRGB *leds, uint16_t numLeds){
      ...
      uint8_t done = 0;  
      for(size_t i = 0; i < METEOR_COUNT; i++){
        ...
        CHSV hsv (meteor.getHue(), 0xFF, 0xFF);
        hsv.hue += bottom;
